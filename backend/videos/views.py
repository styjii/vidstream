...
def video_stream(request, pk):
    ...
    if not range_header:
        ...
        response["Content-Disposition"] = (
            f'inline; filename="{Path(video.file_path).name}"'
        )
...
def video_upload(request):
    ...
    if category_id:
        ...
        category, created = Category.objects.get_or_create(
            name="Uploads",
            defaults={"folder_path": str(dest_dir)},
        )
        if not created and category.folder_path != str(dest_dir):
            category.folder_path = str(dest_dir)
            category.save(update_fields=["folder_path"])
...
def trigger_scan(request):
    ...
    return Response(
        {
            "message": f"{summary['added']} vidéo(s) ajoutée(s).",
